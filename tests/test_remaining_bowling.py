...
def test_remaining_econ_with_no_bowlers_used_defaults_to_league_mean():
    econ, best, n, _ = remaining_bowling_quality({}, legal_balls_bowled=0, stats=stats_with({}))
...
def test_using_up_best_bowler_lifts_remaining_econ():
    ...
    used_all = {"BUMRAH": 24}
    used_none = {"BUMRAH": 0}
    econ_used, _, _, _ = remaining_bowling_quality(used_all, legal_balls_bowled=24, stats=s)
    econ_fresh, _, _, _ = remaining_bowling_quality(used_none, legal_balls_bowled=24, stats=s)
...
def test_best_bowler_overs_left_tracks_freshest_bowler():
    ...
    used = {"A": 12, "B": 6}
    _, best, n, _ = remaining_bowling_quality(used, legal_balls_bowled=18, stats=s)
...
def test_innings_complete_returns_league_mean():
    econ, _, _, _ = remaining_bowling_quality({}, legal_balls_bowled=120, stats=stats_with({}))
    assert abs(econ - LEAGUE_ECON_MEAN) < 1e-6
